...
std::pair<bool, VecXd> OptimizeHelper::parameterFromRandSeed(int randSeed)
{
    ...
    if (taskInfo.dL_dcontrolPoints) {
        ...
        VecXd splineEndPointTranslation = param.controlPointSplines[0][0].segments[0].p1 - param.controlPointSplines[0][0].segments[0].p0;
        ...
        if (taskInfo.dL_dcontrolPoints) {
            ...
            for (int s1 = 0; s1 < splineNum; s1++) {
                ...
                for (int s2 = s1 + 1; s2 < splineNum; s2++) {
                    double dist = (param.controlPointSplines[0][s1].segments[0].p0 - param.controlPointSplines[0][s2].segments[0].p0).norm();
...
double OptimizeHelper::runSimulationAndGetLoss(const VecXd& x)
{
    ...
    double loss = system->runBackwardTask(
                            taskInfo, lossType, lossInfo, statistics, FORWARD_STEPS, param, true,
                            [&](const std::string& v) {}, false)[0]
                      .loss;
    return loss;
}
