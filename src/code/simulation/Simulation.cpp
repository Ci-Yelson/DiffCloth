    ...
    const VecXd& x, double t_now)
{
    ...
    for (int i = 0; i < particles.size(); i++) {
        ...
        if (windEnabled) {
            ...
            if (sceneConfig.windConfig == WindConfig::WIND_SIN_AND_FALLOFF || (sceneConfig.windConfig == WindConfig::WIND_FACTOR_PER_STEP)) {
...
void checkInfoConsistency(int particleId, int otherId, int infoIdx, Simulation::SelfCollisionInformation& info)
{
    ...
    bool assertion = (((info.particleId1 == particleId) && (info.particleId2 == otherId)) || ((info.particleId1 == otherId) && (info.particleId2 == particleId)));
    if (!assertion) {
...
double Simulation::stepFixPoints(double t)
{
    ...
    case TrajectoryConfigs::FIXED_POINT_TRAJECTORY: {
        for (int fixedPointIdx = 0; fixedPointIdx < sysMat[currentSysmatId].fixedPoints.size(); fixedPointIdx++) {
            sysMat[currentSysmatId].fixedPoints[fixedPointIdx].pos = fixedPointTrajectory[forwardRecords.size() - 1].segment(fixedPointIdx * 3, 3);
        }
        break;
    }
    ...
    case TrajectoryConfigs::TRAJECTORY_DRESS_TWIRL: {
        ...
        for (int fixedPointIdx = 0; fixedPointIdx < sysMat[currentSysmatId].fixedPoints.size(); fixedPointIdx++) {
            ...
            sysMat[currentSysmatId].fixedPoints[fixedPointIdx].pos = relativePosRot + rotationCenter; // posLast.cross(Vec3d(0, 1, 0)) * 0.012;
        }
...
void Simulation::step()
{
    ...
    if (!debugSkipfixedpointstep)
        ...
        // Update particles' info according to s_n
#pragma omp parallel for if (OPENMP_ENABLED)
    for (int i = 0; i < particles.size(); i++) {
        Particle& p = particles[i];
...
#pragma omp parallel for if (OPENMP_ENABLED)
                for (int i = 0; i < Constraint::CONSTRAINT_NUM; i++) {
                    returnRecord.At_p_weightless_pertype[i] = sysMat[currentSysmatId].A_t_pertype[i] * projections_pertype[i] / std::sqrt(*(k_stiff_arr[i]));
                }
            }
            ...
            case VELOCITY_BASED: {
                ...
                if (false) {
                    VecXd delta_v_primitives_new = (f_primitives * sceneConfig.timeStep + r_prim).cwiseProduct(m_primitivesinv);
...
#pragma omp parallel for if (OPENMP_ENABLED)
            ...
            if (converged) {
                ...
                returnRecord.cumulateIter = returnRecord.convergeIter + (forwardRecords.empty() ? 0 : forwardRecords[forwardRecords.size() - 1].cumulateIter);
                timeSteptimer.toc();
                break;
            }
            else {
...
#pragma omp parallel for if (OPENMP_ENABLED)
    ...
    returnRecord.accumTimer = Timer::addTimer(returnRecord.timer.timeMicroseconds, forwardRecords[forwardRecords.size() - 1].accumTimer);

    returnRecord.totalRuntime = forwardRecords[forwardRecords.size() - 1].totalRuntime + returnRecord.timer.totalMicroseconds;
...
#pragma omp parallel for if (OPENMP_ENABLED)
...

        {
            ...
            if (backwardGradientForceDirectSolver) {
                ...
                for (int iterIdx = 0; iterIdx < MAX_ITER_NUM; iterIdx++) {
                    ...
                    VecXd deltaU = t_2 * dproj_dxnew_t * (currentSysMat.A * (dr_df_t_x_u_star_prev + u_star_prev)) - currentSysMat.C_t * dr_df_t_x_u_star_prev;
                    VecXd rhs = dL_dxnew + deltaU;
                    u_star = currentSysMat.solver.solve(rhs);
                    bool converged = (std::abs((u_star - u_star_prev).norm() / (particles.size() * 1.0))) < backwardConvergenceThreshold;
    ...
    if (taskInfo.dL_dcontrolPoints && (!currentSysMat.controlPointSplines.empty())) {
        ...
        for (int splineIdx = 0; splineIdx < sysMat[sysMatId].controlPointSplines.size(); splineIdx++) {
            ...
            SpMat rhs_spline = sceneConfig.timeStep * sceneConfig.timeStep * dr_df_plusI * (sysMat[sysMatId].A_t_dp_dxfixed.block(0, s.pFixed * 3, 3 * particles.size(), 3) * dxfixed_dspline_sparse); // 3m x s
    ...
    if (taskInfo.dL_density) {
        VecXd dMy_dd = Area * (forwardInfo_new.x_prev + sceneConfig.timeStep * forwardInfo_new.v_prev + sceneConfig.timeStep * sceneConfig.timeStep * gravity_n);
        VecXd df_dd = Area * (forwardInfo_new.v_prev + sceneConfig.timeStep * gravity_n);
        VecXd rhs = dMy_dd + sceneConfig.timeStep * dr_df * df_dd - Area * forwardInfo_new.x + sceneConfig.timeStep * dr_dd;
    ...
    for (int i = 0; i < Constraint::CONSTRAINT_NUM; i++) {
        ...
        if (taskInfo.dL_dk_pertype[i]) {
            ...
            VecXd rhs = sceneConfig.timeStep * sceneConfig.timeStep * dA_t_times_p_dk + sceneConfig.timeStep * dr_df * df_dk - sceneConfig.timeStep * sceneConfig.timeStep * A_t_A_weightless_times_xnew;
    ...
    if (taskInfo.dL_dfext) {
        dL_dfext_vec = sceneConfig.timeStep * sceneConfig.timeStep * dr_df_plusI.transpose() * u_star * forwardInfo_new.windFactor;
    ...
    if (taskInfo.dL_dfwind) {
        if ((sceneConfig.windConfig != WindConfig::WIND_SIN) && (sceneConfig.windConfig != WindConfig::WIND_SIN_AND_FALLOFF))
...
{
    if ((a < 0) || (b < 0) || (a >= Simulation::sceneConfig.fabric.gridNumY) || (b >= Simulation::sceneConfig.fabric.gridNumX)) {
        return -1;
    }
...
void Simulation::initScene()
{
    ...
    case PLANE_AND_SPHERE: {
        ...
        sphere2.mu = 0.9;
        sphere2.center = sphere2.centerInit = plane1.center + Vec3d(sphere2.radius * 0.3, sphere2.radius, sphere2.radius * 0.1);
    ...
    case SLOPE: {
        ...
        slope.center = slope.centerInit = referencePoint + shift + Vec3d(0, -2, 0);
        slope.mu = 0.2;
    ...
    case SLOPE_SIMPLIEFIED: {
        ...
        slope.center = slope.centerInit = referencePoint + shift + Vec3d(0, -0.3, 0);
...
void Simulation::createClothMeshFromModel(std::vector<Vec3d>& particleIn, std::vector<Vec3i>& meshIn)
{
    ...
    double scale = sceneConfig.fabric.keepOriginalScalePoint ? 1.0 : std::max(std::max(dim[0], dim[1]), dim[2]) / sceneConfig.fabric.clothDimX;
...
void Simulation::loadWindSim2RealAnimationSequence(std::string folderName, std::vector<std::string> files,
    bool isRyanWhite)
{

    std::printf("isRyanWhite: %s\n", isRyanWhite ? "True" : "False");
    sceneConfig.attachmentPoints = CUSTOM_ARRAY;
    std::vector<std::pair<double, std::vector<int>>> ryanWhiteClipIndices = { { 0, { 10, 11, 12, 13, 14, 510, 26, 27, 28, 29, 30, 460, 461, 462, 463, 464, 495 } } };
    ...
    for (int i = 1; i < files.size(); i++) {
        ...
        for (int pIdx = 0; pIdx < particles.size(); pIdx++) {
            x_i.segment(pIdx * 3, 3) = (modelPoints[pIdx] - meshInitTransofrmMinDim) / meshInitTransofrmScale - restShapeMaxDim;
        }
...
double Simulation::calculateLossAndGradient(LossType& lossType, Simulation::LossInfo& lossInfo, VecXd& dL_dx, VecXd& dL_dv,
    int idx, bool calculateLoss)
{
    ...
    case (MATCH_TRAJECTORY): {
        ...
        if (forwardRecords[idx].x.rows() != lossInfo.targetSimulation[idx].x.rows()) {
            std::printf("WARNING: calculate trajectory loss dimension mismatch, but records has size %zu while target has size %zu\n",
        ...
        if (calculateLoss) {
            for (int i = 0; i < forwardRecords.size(); i++) {
                ...
                if (std::isnan(L)) {
                    std::printf("WARNING: Loss becomes NAN after adding subloss from %d in MATCH_TRAJECTORY; record:%.3f target:%.3f\n", i, forwardRecords[i].x.norm(), lossInfo.targetSimulation[i].x.norm());
                }
            }
    ...
    case (DRESS_ANGLE): {
        ...
        double targetHeight = restShapeMinDim[1] + (restShapeMaxDim[1] - restShapeMinDim[1]) * lossInfo.targetTwirlHeight;
...
void Simulation::resetSystemWithParams(Simulation::BackwardTaskInformation& taskConfiguration,
...
void Simulation::calculateFiniteDiffLossArr(Simulation::ParamInfo paramPlus, Simulation::ParamInfo paramMinus, double delta,
...
void Simulation::exportOptimizationRecords(Demos demoIdx, std::string experimentNameGiven, bool useGiven)
{
    ...
    for (int i = 0; i < backwardOptimizationRecords.size(); i++) {
        ...
        std::string iterationFolder = OUTPUT_PARENT_FOLDER + experimentName + "/iter" + std::to_string(i);
        ...
        writeStringToFile(iterationFolder + "/forwardInfo.txt", forwardInfoToString(taskInfo, backwardOptimizationRecords[i].first[backwardOptimizationRecords[i].first.size() - 1]));
    }
}
    ...
    Simulation::BackwardTaskInformation taskInfo, bool writePerf)
{
    if (!statistics.configWritten) {
        std::string experimentName = sceneConfig.name + "-randseed-" + std::to_string(taskInfo.srandSeed) + "-" + currentTimestampToStr() + "-forwardThresh-" + d2str(std::log10(taskInfo.forwardAccuracyLevel), 1);
    ...
    std::string parentFolder = OUTPUT_PARENT_FOLDER + statistics.experimentName + subFolder;
    checkFolderExistsAndCreate(OUTPUT_PARENT_FOLDER);
    checkFolderExistsAndCreate(parentFolder);
    writeStringToFile(parentFolder + "iters.txt",
        "Total forward:" + std::to_string(statistics.totalForwardSim) + "\nTotal backprop:" + std::to_string(statistics.totalBackprop));
    ...
    for (int i = statistics.backwardWritten; i < currentBackwardInfoSize; i++) {
        ...
        appendStringToFile(parentFolder + "backwardLog.txt",
            parameterToString(taskInfo, param) + "Corresponding forward Idx: " + std::to_string(backwardInfo.correspondingForwardIdxInStats) + "\n");
    }
    ...
    for (int i = statistics.optimizationRecordsSaved; i < currentRecordSize; i++) {
        std::string iterationFolder = statistics.experimentName + subFolder + "/iter" + std::to_string(i);
        checkFolderExistsAndCreate(iterationFolder + "/");
    ...
    if (writePerf) {
        ...
        writeStringToFile(
            OUTPUT_PARENT_FOLDER + statistics.experimentName + subFolder + "perf.txt", out);
    }
}
    ...
    Simulation::BackwardInformation grad)
{
    ...
    out += "Backward Non Solve Time[ms]:" + std::to_string(grad.accumSolvePerformanceReport.nonSolveTimeMicroseconds / 1000.0) + "\n";
    out += "Backward Direct-Solve Time[ms]:" + std::to_string(grad.accumSolvePerformanceReport.solveDirectMicroseconds / 1000.0) + "\n";
    out += "Backward Iter-Solve Time[ms]:" + std::to_string(grad.accumSolvePerformanceReport.solveIterativeMicroseconds / 1000.0) + "\n";
    ...
    out += "Direct-Solve[ms]:" + d2str(grad.accumSolvePerformanceReport.solveDirectMicroseconds / 1000.0, 5) + "\t|" + d2str(percentageDirect * 100, 3) + "%\n";
    out += "Iter-Solve[ms]:" + d2str(grad.accumSolvePerformanceReport.solveIterativeMicroseconds / 1000.0, 5) + "\t|" + d2str(percentageIter * 100, 3) + "%\n";
