...
double Triangle::evaluateEnergy(const Mat3x2d& F, const VecXd& x_new)
{
    ...
    case NON_QUADRATIC:
        ...
        double m_energy = area_rest * (k[0] * G1(0, 0) * G1(0, 0) + k[2] * G1(1, 1) * G1(1, 1) + k[1] * G1(0, 0) * G1(1, 1) * 2.0f + k[3] * G1(0, 1) * G1(0, 1)) / 2.0f;
...
Vec9d Triangle::stretchingForce(const VecXd& x_new)
{
    ...
    case QUADRATIC: {
        ...
        stretchingForceBuffer = -k_stiff * area_rest * P_vec.transpose() * (dF_dx.transpose() - dproj_dx.transpose()).transpose();
...
Mat9x9d Triangle::stretchingHessian(const VecXd& x_new) const
{
    ...
    case NON_QUADRATIC: {
        ...
        Mat9x9d hess_e = k[0] * (outer(fuu, fuu) + std::fmax(e_uu, 0.) * Du.transpose() * Du) // Jacobian of force
            ...
            + k[1] * ((outer(fuu, fvv)) + std::fmax(e_uu, 0.) * Dv.transpose() * Dv + (outer(fvv, fuu)) + std::fmax(e_vv, 0.) * Du.transpose() * Du)
            + 2. * k[3] * (outer(fuv, fuv));
...
Triangle::Triangle(int p0_idx, int p1_idx, int p2_idx, std::vector<Particle>& pArr, bool checkArea)
    : Constraint(
        Constraint::ConstraintType::CONSTRAINT_TRIANGLE, 6)
    , p0_idx(p0_idx)
    , p1_idx(p1_idx)
    , p2_idx(p2_idx)
    , pArr(pArr)
